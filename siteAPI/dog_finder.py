...
def get_requests(parameter: str, value: str, choice_sort: str, reverse=False):
    ...
    response = requests.get(api_url + params, headers={'X-Api-Key': key_api.site_api.get_secret_value()})
    data = response.json()
    data_sort = sorted(data, key=lambda val: val[f'min_{choice_sort}_female'], reverse=reverse)
    return data_sort
