...
def hello(message):
    ...
    bot.send_message(
        message.chat.id,
        "Привет, я бот, который поможет подобрать собаку, в зависимости от характеристик.\n"
...
def low(message):
    ...
    min_max = False
    bot.send_message(
        message.chat.id, "В этом меню поиск идет по минимальным значениям: вес и рост."
    )
    bot.send_message(
        message.chat.id, "Сделайте выбор\nсортировки поиска:", reply_markup=kb_sort
    )
...
def high(message):
    ...
    min_max = True
    bot.send_message(
        message.chat.id, "В этом меню поиск идет по максимальным значениям: вес и рост."
    )
    bot.send_message(
        message.chat.id, "Сделайте выбор\nсортировки поиска:", reply_markup=kb_sort
    )
...
def er_answer(message):
    ...
    bot.send_message(
        message.chat.id, "Я вас не понимаю.\nИспользуйте меню\nи доступные команды."
    )
        ...
        if int(count_dogs) in range(21):
            ...
            for item in data[: int(count_dogs)]:
                if isinstance(item, dict):
                    bot.send_message(
                        message.chat.id, back_message(item), parse_mode="HTML"
                    )
                    History.create(
                        user_id=message.chat.id, user_resp=back_message(item)
                    )

        else:
            bot.send_message(
                message.chat.id, "Неверный ввод!\nВведите число от 1 до 20"
            )
...
def get_custom_weight(message):
    ...
    try:
        ...
        if min_wght.isdigit() and max_wght.isdigit():
            ...
            bot.send_message(
                message.chat.id,
                "Вводимый текст должен быть цифрой.\n"
...
def action(call):
    ...
    if call.message:
        if call.data == "weight" or call.data == "height":
            ...
            call.data == "shedding"
            or call.data == "trainability"
            or call.data == "barking"
            or call.data == "protectiveness"
        ):
            choice_value = call.data
            bot.send_message(
                call.message.chat.id,
                f"Выберите значение {call.data} от 1 до 5",
            ...
            grade = call.data
            bot.send_message(
                call.message.chat.id, "Сколько показать собак?(не больше 20)"
            )
            ...
            bot.send_message(
                call.message.chat.id,
                "Введите минимальный и максимальный рост через пробел.",
            )
